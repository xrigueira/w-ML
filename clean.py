...
class Model():
    ...
    The difference with the program in main.py is that this program trains the algorithm on the clean dataset 
    and performs prediction on the whole dataset. Whereas, the program in the main.py file trains the algorithm 
    works on the whole dataset, including the anomalies not detected by the functional model from w-fda (MSA)
    """

    def __init__(self, model_name, station, window_size, stride, threshold, search) -> None:
    ...
    def windower_prediction(self):
        ...
        # Save the inference data to disk
        np.save(f'X_{self.station}.npy', X, allow_pickle=False, fix_imports=False)
    ...
    def predictor(self):
        ...
        # np.save(f'y_{self.model_name}_{self.station}.npy', y_hat, allow_pickle=False, fix_imports=False)
        # np.save(f'y_gt_ml_{self.station}.npy', self.y_pred, allow_pickle=False, fix_imports=False)

if __name__ == '__main__':
    
    # Create an instance of the class
    model = Model(model_name='rf', station=901, window_size=32, stride=1, threshold=0.65, search=False)
